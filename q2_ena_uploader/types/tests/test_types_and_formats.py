...
from q2_ena_uploader.types import (
    ENAMetadataSamples,
...
class TestTypes(TestPluginBase):
    package = "q2_ena_uploader.types.tests"
...
class TestFormats(TestPluginBase):
    ...
    def test_ena_study_missing_values(self):
        ...
        with self.assertRaisesRegex(
            ValidationError,
            "The study is missing values in the following fields: " "alias,title.",
        ):
            format.validate()
...
class TestTransformers(TestPluginBase):
    ...
    def setUp(self):
        ...
        self.ena_meta_study_df = pd.read_csv(
            meta_path2, header=None, index_col=0, sep="\t"
        )
    ...
    def test_ena_samples_metadata_to_df(self):
        _, obs = self.transform_format(
            ENAMetadataSamplesFormat, pd.DataFrame, "ena_metadata_samples.tsv"
        )
    ...
    def test_ena_study_metadata_to_dict(self):
        _, obs = self.transform_format(
            ENAMetadataStudyFormat, dict, "ena_metadata_study.tsv"
        )
    ...
    def test_ena_samples_meta_to_q2_meta(self):
        ...
        ena_meta_samples = ena_meta_samples.rename(columns={"alias": "id"}).set_index(
            "id"
        )
    ...
    def test_ena_study_meta_to_q2_meta(self):
        ...
        ena_meta_study = ena_meta_study.T.rename(columns={"alias": "id"}).set_index(
            "id"
        )
