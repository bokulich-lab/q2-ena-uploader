...
class Run:
    def __init__(self, alias=None, center_name=None, experiment_ref=None, files=[]):
        self.alias = alias
    ...
    def to_xml_element(self):
        ...
        if self.alias is None:
            ...
            run_element = ET.SubElement(
                root, "RUN", {"alias": self.alias, "center_name": self.center_name}
            )

        if self.experiment_ref is None:
            raise ValueError(
                "Experiment reference must be present for a run submission."
            )
        else:
...
def _runFromRawDict(rowDict):
    ...
    kwArgs["files"] = {
        k: v
        for k, v in rowDict.items()
        if k.startswith("file") or k.startswith("checksum") or k.startswith("read_type")
    }
    return Run(**kwArgs)
