...
class Library:
    ...
    def to_xm_element(self):
        if self.library_strategy is None:
            raise ValueError(
                "Library strategy must be present for an experiment submission."
            )
        else:
        ...
        if self.library_construnction_protocol is not None:
            ET.SubElement(root, "LIBRARY_CONSTRUCTION_PROTOCOL").text = str(
...
class Experiment:
    ...
    def to_xml_element(self):
        ...
        if self.sample_description is None:
            raise ValueError(
                "Sample reference id must be present for an experiment submission."
            )
        else:
            root = ET.Element(
        ...
        if self.study_ref is None:
            raise ValueError(
                "Study reference must be present for an experiment submission."
            )
        else:
...
def _experimentFromRowDict(rowDict):
    ...
    kwArgs["library_attributes"] = {
        k: v for k, v in rowDict.items() if k.startswith("library")
    }
    kwArgs["attributes"] = {
        k: v
        for k, v in rowDict.items()
        if k not in specialAttributes and not k.startswith("library")
    }
