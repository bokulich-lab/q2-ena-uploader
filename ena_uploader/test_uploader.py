...
from ena_uploader import register_metadata, cancel_object_submission, ActionType

class TestENAUploader(unittest.TestCase):

    @patch('os.getenv')
    @patch('requests.post')
    def test_register_metadata(self, mock_post, mock_getenv):
        ...
        result = register_metadata(study=mock_study, samples=mock_samples, submission_hold_date="2024-10-01", action_type="ADD", dev=True)
    ...
    @patch('requests.post')
    @patch('os.getenv')
    def test_cancel_object_submission(self, mock_getenv, mock_post):
        mock_getenv.side_effect = lambda key: {'ENA_USERNAME': 'test_user', 'ENA_PASSWORD': 'test_pass'}.get(key)
        ...
        result = cancel_object_submission(accession_number="ABC123", dev=True)
